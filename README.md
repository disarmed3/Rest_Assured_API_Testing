...
## Overview
This project contains automated API tests for the Books API written in Java, using Rest Assured for HTTP request/response testing and JUnit 5 for test execution. Tests are built with Maven. The suite validates endpoint behavior including happy paths, negative cases, JSON schema/contract checks, edge cases, and basic smoke assertions. Configuration (base URI, tokens, etc.) is provided via resource files or system properties so tests can run locally or in CI.
